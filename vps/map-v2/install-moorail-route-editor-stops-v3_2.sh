...
BACKUP="$ROOT/backups/route-editor-stops-v3_2-$STAMP"
TMP="$(mktemp -d /tmp/moorail-stops-v32.XXXXXX)"
...
rollback(){
  ...
  echo "ROLLBACK STOP-TO-STOP V3.2..." >&2
...
echo "============================================================"
echo " MOO RAIL ROUTE EDITOR V3.2 — ARRÊT -> ARRÊT"
...
checks={
  ...
  'routage': 'function routeAnchors' in js and 'async function recompute' in js and 'new Heap()' in js,
  'validation': 'function validateLeg' in js,
...
assert 'function routeAnchors' in js and 'async function recompute' in js
...
echo
echo "============================================================"
echo " V3.2 ARRÊT -> ARRÊT INSTALLEE"
echo "============================================================"
